...
- **Backend Proxy:** Express.js server for secure Notion API access.
- **Persistent Storage:** Notion API (databases & pages) for storing meeting transcripts.
...
7. **Save to Notion:** When stopping a recording, a modal opens allowing the user to save the transcript to a Notion database. Title and tags are auto-generated by AI but fully editable. Transcripts are organized by project (database) and category (Daily, Refinement, etc.).
8. **Search:** A dedicated search page lets users query past transcripts in natural language. Keywords are extracted via Gemini, used to search Notion, and results are analyzed by AI to provide direct answers.
...
- **`Header.js` / `Footer.js`**: Reusable Vanilla JS components injected dynamically into all pages (dashboard, summary, search) for consistent, DRY UI layout.
- **`SaveModal.js`**: Glassmorphism modal triggered when stopping a recording. Auto-generates title and tags via Gemini, loads project databases and categories from Notion, and lets the user save or download the transcript. Remembers last-used project and category in localStorage.

### Frontend Services (`src/services/`)
- **`notion-client.js`**: Frontend API client for the Express proxy. Provides async functions for all Notion operations (health check, list databases, create database, get categories, save transcript, search transcripts) without ever exposing the Notion secret.

### Backend Proxy (`server/`)
- **`index.js`**: Express server on port 3001. Serves as a secure proxy between the browser and the Notion API. Endpoints: health, databases CRUD, pages CRUD, search. Uses CORS, JSON body parsing, and request logging.
- **`notion.js`**: Notion service layer following the repository pattern. Handles database creation with schema (title, category, tags, date, duration, entries), page creation with Notion content blocks (splitting long text into 2000-char chunks), and full-text search with keyword matching and excerpt extraction.

### Pages (`pages/`)
- **`summary.html`**: AI-generated meeting summary page.
- **`search.html`**: AI-powered search interface for querying past transcripts across Notion databases.
...
- **Crash Recovery & Auto-Save:** To prevent data loss from accidental tab closures, the `TranscriptManager` periodically snapshots its state to `localStorage` (debounced every 5s). A `beforeunload` handler warns the user and forces a final save. On next load, if a stale auto-save is detected, a recovery banner slides in, letting the user restore the transcript into the SaveModal or discard it. The auto-save is cleared on normal stop-recording flow.

## 6. Development & Run Instructions

- **Install:** `npm install`
- **Run Dev Server:** `npm run dev` (starts both Vite and the Express proxy via `concurrently`)
- **Run Vite only:** `npm run dev:vite`
- **Run Express only:** `npm run dev:server`
- **Build:** `npm run build`

*Note: For the application to function correctly, the browser must support WebGPU and Web Workers. The AI Chat's local mode requires Ollama to be running on `http://localhost:11434`. The Notion integration requires `NOTION_API_SECRET` and `NOTION_PARENT_PAGE_ID` in `.env`.*
